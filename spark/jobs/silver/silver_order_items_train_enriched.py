...
def create_spark_session() -> SparkSession:
    spark = (
        ...
        .appName("retail-lakehouse-silver-order-items-train-enriched")
        .master("local[*]")
...
def build_silver_order_items_train_enriched(
    order_products_train_df: DataFrame,
    orders_clean_df: DataFrame,
    products_enriched_df: DataFrame,
) -> DataFrame:
    print("==================================================")
    print("Transformação Silver de order_items_train_enriched")
    print("==================================================")

    silver_df = (
        order_products_train_df.alias("op")
        .join(
...
def write_silver_order_items_train_enriched(df: DataFrame, output_dir: Path) -> None:
    print("==================================================")
    print("Escrita da Silver de order_items_train_enriched")
    print("==================================================")
    ...
    print("[OK] Silver de order_items_train_enriched escrita com sucesso em Parquet.")


def summarize_silver_output(df: DataFrame, output_dir: Path) -> None:
    print("==================================================")
    print("Resumo da Silver de order_items_train_enriched")
    print("==================================================")
...
def run_silver_order_items_train_enriched() -> None:
    print("==================================================")
    print("Retail Lakehouse - Silver de order_items_train_enriched")
    print("==================================================")

    paths = resolve_container_data_paths()

    order_products_train_dir = paths["bronze_dir"] / "instacart" / "order_products_train"
    ...
    latest_order_products_train_partition = find_latest_partition(order_products_train_dir)
    ...
    silver_ingestion_date = extract_ingestion_date_from_partition(
        latest_order_products_train_partition
    )

    output_dir = (
        paths["silver_dir"]
        / "instacart"
        / "order_items_train_enriched"
    ...
    try:
        order_products_train_df = read_dataset(
            spark=spark,
            path=latest_order_products_train_partition,
            dataset_name="order_products_train",
        )
        ...
        silver_df = build_silver_order_items_train_enriched(
            order_products_train_df=order_products_train_df,
            orders_clean_df=orders_clean_df,
            products_enriched_df=products_enriched_df,
        )
        write_silver_order_items_train_enriched(df=silver_df, output_dir=output_dir)
        summarize_silver_output(df=silver_df, output_dir=output_dir)

        print("[OK] Job Silver de order_items_train_enriched concluído com sucesso.")
    finally:
        spark.stop()


if __name__ == "__main__":
    run_silver_order_items_train_enriched()