...
def create_spark_session() -> SparkSession:
    ...
    spark = (
        ...
        .appName(f"{spark_app_name}-bronze-order-products-train")
        .master("local[*]")
...
def read_raw_order_products_train(spark: SparkSession, raw_csv_path: Path) -> DataFrame:
    print("==================================================")
    print("Leitura da raw de order_products_train")
    print("==================================================")
    print(f"Arquivo de entrada: {raw_csv_path}")

    if not raw_csv_path.exists():
        raise FileNotFoundError(f"Arquivo raw não encontrado: {raw_csv_path}")
...
def transform_order_products_train_to_bronze(
    df: DataFrame,
    raw_ingestion_date: str,
) -> DataFrame:
    print("==================================================")
    print("Transformação Bronze de order_products_train")
    print("==================================================")
...
def write_bronze_order_products_train(df: DataFrame, bronze_output_dir: Path) -> None:
    print("==================================================")
    print("Escrita da Bronze de order_products_train")
    print("==================================================")
    ...
    print("[OK] Bronze de order_products_train escrita com sucesso em Parquet.")


def summarize_bronze_output(df: DataFrame, bronze_output_dir: Path) -> None:
    print("==================================================")
    print("Resumo da Bronze de order_products_train")
    print("==================================================")
...
def run_bronze_order_products_train() -> None:
    print("==================================================")
    print("Retail Lakehouse - Bronze de order_products_train")
    print("==================================================")

    paths = resolve_container_data_paths()

    raw_entity_dir = paths["raw_dir"] / "instacart" / "order_products_train"
    ...
    raw_csv_path = latest_raw_partition / "order_products__train.csv"
    ...
    try:
        raw_df = read_raw_order_products_train(spark=spark, raw_csv_path=raw_csv_path)
        bronze_df = transform_order_products_train_to_bronze(
            df=raw_df,
            raw_ingestion_date=raw_ingestion_date,
        )
        write_bronze_order_products_train(
            df=bronze_df,
        ...
        print("[OK] Job Bronze de order_products_train concluído com sucesso.")
    finally:
        spark.stop()


if __name__ == "__main__":
    run_bronze_order_products_train()