...
def validate_gold_orders_by_period() -> None:
    print("==================================================")
    print("Retail Lakehouse - Validação Gold orders_by_period")
    print("==================================================")

    gold_base_dir = resolve_gold_base_dir()
    entity_dir = gold_base_dir / "instacart" / "orders_by_period"
    ...
    print("==================================================")
    print("Resultado da validação Gold orders_by_period")
    print("==================================================")
    ...
    if row_count == 0:
        errors.append(
            "A Gold de orders_by_period está vazia no arquivo analisado."
        )

    if missing_columns:
        errors.append(
            f"Colunas ausentes na Gold de orders_by_period: {missing_columns}"
        )
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        raise ValueError("Validação da Gold de orders_by_period falhou.")

    print("[OK] Validação da Gold de orders_by_period concluída com sucesso.")